...
def manage_wallets():
    ...
    if request.method == 'GET':
        wallets = Wallet.query.filter_by(user_id=user_id, is_deleted=False).all()
...
@foundation_bp.route('/api/wallets/<int:wallet_id>', methods=['PUT', 'DELETE'])
@api_login_required
def modify_wallet(wallet_id):
    user_id = session['user_id']
    wallet = Wallet.query.filter_by(id=wallet_id, user_id=user_id, is_deleted=False).first()
